...
def calculate_gamma_for_collision_pair(collision_index, collision_pair, scen_properties, debris_species, binE_mass, LBgiven):
    ...
    # This will create fragment plots for each of the different implementation. 
    # plot_fragments(fragments, collision_index, m1, m2, scen_properties)

    return collision_pair

import os

def plot_fragments(fragments, collision_index, m1, m2, scen_properties):
    """
    Visualize the fragments as a 3D surface plot and save the figure in subfolders for each unique (m1, m2) pair.
    """
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111, projection='3d')
    
    num_shells, num_properties = fragments.shape
    shell_altitudes = scen_properties.HMid  # Assumed to be an array of same length as rows
    
    X, Y = np.meshgrid(range(num_properties), shell_altitudes)
    Z = np.array(fragments)
    
    surf = ax.plot_surface(X, Y, Z, cmap='viridis', edgecolor='none')
    ax.set_xlabel("Fragment Index")
    ax.set_ylabel("Shell Altitude (km)")
    ax.set_zlabel("Fragment Mass/Property")
    fig.colorbar(surf, label='Fragment Value')
    
    # Create subfolders for each unique (m1, m2) pair
    base_folder = './figures/collision_tests'
    m1_folder = os.path.join(base_folder, f'm1_{m1}')
    m2_folder = os.path.join(m1_folder, f'm2_{m2}')
    
    os.makedirs(m2_folder, exist_ok=True)  # Ensures both m1 and m2 folders exist
    
    save_path = os.path.join(m2_folder, f'Collision_{collision_index}_m1_{m1}_m2_{m2}.png')
    
    ax.set_title(f"Collision {collision_index}: m1={m1} kg, m2={m2} kg")
...
def evolve_bins_nasa_sbm(scen_properties, m1, m2, r1, r2, v, binE_mass, collision_index):
    ...
    for attempt in range(attempts):
        try:
            debris1, debris2, isCatastrophic = frag_col_SBM_vec_lc2(0, p1_in, p2_in, param, scen_properties.LC, scen_properties.min_altitude + 6378)
            break  # If successful, break out of the loop
        except Exception as e:
            print(f"Attempt {attempt + 1} failed: {e}")
            if attempt == attempts - 1:
                print(f"No fragments available for {m1}, {m2} at shell index {collision_index}")
                return None
